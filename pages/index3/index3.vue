			...
			>
				...
				<text class="title clamp">{{item.goods_name}}</text>
					...
					<view class="price-box">
						...
						<text class="sales_sum">已销{{item.sales_sum}}</text>
					</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			this.getList();
			util.request(api.getAdListInfo,{position_desc: 'goods_ad'}).then(
				res => {
					console.log(res)
		...
		methods: {
			...
			getList() {
				page++
				var that = this
				util.request(api.recommendGoodsList,{page: page,limit: 10,}).then(
					res => {
						console.log(res)
